...
namespace ProCoder.Areas.Admin.Controllers
{
    ...
    public class HomeAdminController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            if (adminCount > 0)
                roleDistribution.Add(new RoleDistributionItem { Role = "Admin", Count = adminCount });

            // Đếm số Contest Setter (không phải admin)
            var setterCount = _context.Coders.Count(c => c.ContestSetter && !c.AdminCoder);
            if (setterCount > 0)
                roleDistribution.Add(new RoleDistributionItem { Role = "Contest Setter", Count = setterCount });
            ...
            if (userCount > 0)
                roleDistribution.Add(new RoleDistributionItem { Role = "User", Count = userCount });
